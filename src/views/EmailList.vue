<template>
  <div>
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
      <div>
        ...
        <p class="text-gray-600">
          ...
          et {{ sentEmails.length }} e-mail{{ sentEmails.length > 1 ? 's' : '' }} envoyés.
        </p>
      </div>
    ...
    <!-- E-mails envoyés -->
    <div class="card p-6 mb-6">
      <h2 class="text-lg font-semibold mb-4">E-mails envoyés</h2>
      <div v-if="sentEmails.length === 0" class="text-center py-6">
        <p class="text-gray-600">Aucun e-mail envoyé pour le moment.</p>
      </div>
      <div v-else class="space-y-3">
        <div
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <h3 class="text-lg font-medium text-gray-900 truncate">{{ email.subject }}</h3>
              <p class="text-sm text-gray-600">À: {{ email.recipient.name }} <{{ email.recipient.email }}></p>
              <p class="text-sm text-gray-500">Envoyé le: {{ formatDate(email.sentDateTime) }}</p>
            </div>
          </div>
        </div>
      </div>
...
const sentEmails = computed(() => emailStore.sentEmails.value);
