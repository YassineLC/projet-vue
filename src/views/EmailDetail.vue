      ...
      <div class="p-6 border-b border-gray-200">
        <div class="flex items-start justify-between mb-4">
          ...
          <div class="flex items-center space-x-2 ml-4">
            ...
            <button
              @click="handleHideEmail"
              class="p-2 text-gray-400 hover:text-red-600 transition-colors"
              title="Masquer cet e-mail"
            >
...
const loading = computed(() => emailStore.loading.value); // Utilisation de loading depuis emailStore
...
const sanitizedBody = computed(() => {
  ...
  // Basique sanitisation - dans un vrai projet, utiliser DOMPurify
...
/**
 * Retourne à la liste des e-mails
 */
const goBack = () => {
  router.push('/emails')
}
...
const handleMarkAsRead = async () => {
  if (email.value) {
    ...
    // Met à jour l'email local
    email.value.isRead = true
  }
}

/**
 * Masque l'e-mail et retourne à la liste
 */
const handleHideEmail = async () => {
  if (email.value) {
    const result = await emailStore.hideEmail(email.value.id)
    if (result.success) {
      goBack()
    }
  }
...
/**
 * Charge l'e-mail au montage si pas en cache
 */
onMounted(async () => {
  try {
    loading.value = true;
    const fetchedEmail = await emailStore.getEmailById(props.id);
    email.value = fetchedEmail || null;
    if (!email.value) {
      console.error('Erreur: L\'e-mail demandé n\'existe pas.');
    }
  } catch (error) {
